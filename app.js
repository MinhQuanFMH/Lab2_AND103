...
const sinhvien=require('./sinhvienModel');//import module sinh vien

//b2: tao doi tuong server
const app=express();
app.set('view engine','ejs');//view la 1 file ejs
...
}).then(()=>{//ket qua tra ve sau khi ket noi csdl
    console.log("Ket noi thanh cong voi Mongodb")
}).catch((err)=>{//truong hop loi thi in ra loi
    console.error(err);
});
...
app.get('/sinhvien', async (req,res)=>{//su dung phuong thuc GET du lieu voi duong dan /sinhvien
    ...
    try {
        ...
        res.render('students',{sinhviens: sinhviens});//goi view
...
app.listen(PORT,()=>{//khoi dong server
